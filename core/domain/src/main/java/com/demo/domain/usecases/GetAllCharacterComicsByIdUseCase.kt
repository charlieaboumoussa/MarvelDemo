...
@Singleton
class GetAllCharacterComicsByIdUseCase @Inject constructor(
    private val charactersRepository: CharacterRepository
) {

    operator fun invoke(id: Int, offset: Int, limit: Int): Flow<Resource<List<CharacterComic>>> = flow {
        emit(Resource.Loading())
        try {
            charactersRepository.getCharacterComics(id, offset, limit).collect {
